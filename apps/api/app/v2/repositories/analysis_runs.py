...
class AnalysisRunV2Repository:
    ...
    def mark_stale_due_to_assessment_change(self, user_id: str) -> int:
        with session_scope() as session:
            records = list(
                session.scalars(
                    select(AnalysisRunRecord)
            ...
            for record in records:
                record.freshness_status = FreshnessState.STALE_DUE_TO_ASSESSMENT_CHANGE.value
                record.rerun_required = True
                record.rerun_reason = RerunReason.ASSESSMENT_CHANGED.value
